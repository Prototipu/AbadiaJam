        ...
        private void GetTooltipInfo(string keyword, Vector3 mousePos)
        {
            foreach (var entry in tooltipContentList)
            {
                if (entry.Keyword == keyword)
                {
                    ...
                    {
                        tooltipContainer.transform.position = mousePos +  new Vector3(0, -50, 0); // This offset is an example, you'll probably need to find your own best fitting value.
                        tooltipContainer.gameObject.SetActive(true);
                    }
                    
                    _tooltipDescriptionTMP.text = entry.Description;
                    _tooltipImage.sprite = entry.Image;
                    return;
                }
            }
